...
export async function createEvent(
  ...
  try {
    ...
    if (!startsAtRaw || Number.isNaN(startsAt.getTime()))
      return { error: "Укажите дату и время начала." };
    if (startsAt.getTime() <= Date.now())
      return { error: "Событие можно назначить только на будущее время." };
    ...
    if (scope === "local" && !profile?.city_id)
      return { error: "Сначала выберите город для локального события." };

    let verifiedPlaceId: string | null = null;
    if (placeId) {
      if (scope !== "local")
        return { error: "Место можно привязать только к городскому событию." };
      ...
      if (!place || !place.is_active || place.city_id !== profile?.city_id)
        return { error: "Выберите действующее место из своего города." };
    ...
    const { data: event, error } = await supabase
      ...
      .single();
    if (error) return { error: `Не удалось создать событие: ${error.message}` };
    ...
    return {
      error:
        err instanceof Error && err.message
          ? err.message
          : "Не получилось создать событие. Попробуй ещё раз.",
    };
  }
}

...
**Цель:** поднять тестовый контур в российской инфраструктуре, применить 94 миграции, прогнать сценарии QA и войти в закрытую бета-фазу Будённовска. Ниже — точный порядок действий. Никаких секретов в чат: env заполняются только локально на сервере.
...
- [ ] Применить миграции: `npm run qa:migrations` (должно показать 94, без пропусков).
